...
optimal_clusters_euclidean <- kmeansruns(k.mat, krange = 2:5, critout=TRUE, criterion = "asw",
                                         iter.max = 100, nstart = 100 , alpha=0.001 ) 
# 'nstart' is the number of random initializations, similar to 'runs' in kmeansCBI

# Extracting clusters and centers
clusters_euclidean_runs <- optimal_clusters_euclidean$cluster
centers_euclidean_runs <- optimal_clusters_euclidean$centers

# Visualization (Euclidean)
fviz_cluster(list(data = k.mat, cluster = clusters_euclidean_runs), 
             geom = "point", stand = FALSE, frame.type = "norm") + theme_bw()

plotcluster(k.mat, clusters_euclidean_runs)
dp <- discrproj(k.mat, clusters_euclidean_runs)
plot(dp$proj[,1], dp$proj[,2], pch = clusters_euclidean_runs + 48, col = clusters_euclidean_runs)

# Prepare data for export (Euclidean)
test1 <- list(kmean_r[["id_agree"]]  , clusters_euclidean_runs, dp$proj[,1], dp$proj[,2])
res_1 <- as.data.frame(do.call(cbind, test1))
...
fviz_nbclust(k.mat, hcut,   method = "silhouette")

################################################################################
################################################################################

res <- NbClust(k.mat,  distance = "euclidean", min.nc=2, max.nc=10, method = "kmeans", index = "silhouette")
res$All.index
res$Best.nc
res$Best.partition
...
test3    <- list(rownames(df)  , res[["Best.partition"]],   k3[["cluster"]],  k3_scaled[["cluster"]], clusters_euclidean_runs, clusters_pam)
