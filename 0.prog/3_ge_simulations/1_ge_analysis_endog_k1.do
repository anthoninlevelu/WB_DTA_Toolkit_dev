...
local extx  : dir "`c(pwd)'"   files "*EXT1*.xls" 
...
collapse (min) avg_rank rank*, by(iso)
...
export excel using 	"$RES\\`iso'\\RTA_to_sign.xlsx", sheet("RTA to sign_cty_rank")  sheetreplace firstrow(variables) nolabel 
...
drop _m
		...
		export excel using 	"`counterfactual'_EXT1_`X'_AM.xls", firstrow(variables) replace
	    save 				"`counterfactual'_EXT1_`X'_AM.dta", replace
