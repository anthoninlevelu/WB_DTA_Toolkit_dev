...
local extx  : dir "`c(pwd)'"   files "*EXT3*.xls" 
...
replace  rta_k3_cfl  		= 1  if ( iso_o == "`iso'"    			 & iso_d == "`counterfactual'" )   & (exporter!= importer)    & ($rta == 0 | ($rta == 1 & strpos(agreement, "GSTP")))
replace  rta_k3_cfl  		= 1  if ( iso_o == "`counterfactual'"    & iso_d == "`iso'"            )   & (exporter!= importer)    & ($rta == 0 | ($rta == 1 & strpos(agreement, "GSTP")))
...
replace  rta_k3_cfl  		= 1  if ( iso_o == "`iso'"    			 & eu_d  == 1        )   & (exporter!= importer)    & ($rta == 0 | ($rta == 1 & strpos(agreement, "GSTP")))
replace  rta_k3_cfl  		= 1  if ( eu_o  == 1                     & iso_d == "`iso'"  )   & (exporter!= importer)    & ($rta == 0 | ($rta == 1 & strpos(agreement, "GSTP")))
...
drop _m
		...
		export excel using 	"`counterfactual'_EXT3_`X'_AM.xls", firstrow(variables) replace
	    save 				"`counterfactual'_EXT3_`X'_AM.dta", replace
