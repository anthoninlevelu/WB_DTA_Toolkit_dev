...
cap 	log close
capture log using "$PROG\00_log_files\ge_exd_`=$iso'", text replace
...
local exdx  : dir "`c(pwd)'"   files "*EXD*.xls" 
...
cap 	log close  
capture log using "$PROG\00_log_files\ge_extd_`=$iso'", text replace
...
keep iso_o iso_d pair_id t decade exporter importer trade est_* tij_* gamma_* INTL_BRDR_* rta rta_* region_* id_agree
 cd "$RES\\`iso'\\temp"
...
replace  rta_k1_cfl  	   = 1  if ( iso_o    == "`iso'"     & multi_d  == 1     )   & (exporter!= importer)  
replace  rta_k1_cfl  	   = 1  if ( multi_o  == 1           & iso_d == "`iso'"  )   & (exporter!= importer)  
...
replace  rta_k1_cfl  	   = 1  if ( eu_o     == 1        & multi_d  == 1     )   & (exporter!= importer)  
replace  rta_k1_cfl  	   = 1  if ( multi_o  == 1        & eu_d     == 1     )   & (exporter!= importer)  
...
replace  rta_k1_cfl  	   = 1  if ( iso_o    == "`iso'"     & rcep_d  == 1        )   & (exporter!= importer)  
replace  rta_k1_cfl  	   = 1  if ( rcep_o   == 1           & iso_d   == "`iso'"  )   & (exporter!= importer)  
...
replace multi_o			   = 1 if (iso_o =="BGD") |  (iso_o =="EGY") | (iso_o =="IDN") | (iso_o =="IRN") | (iso_o =="NGA") | (iso_o =="PAK") | (iso_o =="TUR")
replace multi_d			   = 1 if (iso_d =="BGD") |  (iso_d =="EGY") | (iso_d =="IDN") | (iso_d =="IRN") | (iso_d =="NGA") | (iso_d =="PAK") | (iso_d =="TUR")

replace  rta_k1_cfl  	   = 1  if ( multi_o  == 1           & multi_d  == 1  )   & (exporter!= importer)  
...
drop _m
		...
		export excel using 	"`counterfactual'_EXD_`X'_AM.xls", firstrow(variables) replace
	    save 				"`counterfactual'_EXD_`X'_AM.dta", replace
