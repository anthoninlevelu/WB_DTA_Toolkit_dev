...
cap 	log close
capture log using "$PROG\00_log_files\ge_exd3_`=$iso'", text replace
...
local exdx  : dir "`c(pwd)'"   files "*EXD3*.xls" 
...
cap 	log close  
capture log using "$PROG\00_log_files\ge_extd3_`=$iso'", text replace
...
replace  rta_k3_cfl  		= 1  if ( iso_o == "`iso'"    			 & iso_d == "`counterfactual'" )   & (exporter!= importer)  & rta == 0
replace  rta_k3_cfl  		= 1  if ( iso_o == "`counterfactual'"    & iso_d == "`iso'"            )   & (exporter!= importer)  & rta == 0
...
replace  rta_k3_cfl  		= 1  if ( iso_o == "`iso'"    			 & eu_d  == 1        )   & (exporter!= importer)   & rta == 0
replace  rta_k3_cfl  		= 1  if ( eu_o  == 1                     & iso_d == "`iso'"  )   & (exporter!= importer)   & rta == 0
...
replace  rta_k3_cfl  	   = 1  if ( iso_o    == "`iso'"     & multi_d  == 1     )   & (exporter!= importer)   & rta == 0
replace  rta_k3_cfl  	   = 1  if ( multi_o  == 1           & iso_d == "`iso'"  )   & (exporter!= importer)   & rta == 0
...
replace  rta_k3_cfl  	   = 1  if ( iso_o    == "`iso'"  & multi_d  == 1       )   & (exporter!= importer)   & (rta == 0 | (rta == 1 & strpos(agreement, "GSTP")))
replace  rta_k3_cfl  	   = 1  if ( multi_o  == 1        &  iso_d    == "`iso'")   & (exporter!= importer)   & (rta == 0 | (rta == 1 & strpos(agreement, "GSTP")))
}
...
replace  rta_k3_cfl  	   = 1  if ( iso_o    == "`iso'"     & rcep_d  == 1        )   & (exporter!= importer)  // & rta == 0
replace  rta_k3_cfl  	   = 1  if ( rcep_o   == 1           & iso_d   == "`iso'"  )   & (exporter!= importer)  // & rta == 0
...
drop _m
		...
		export excel using 	"`counterfactual'_EXD3_`X'_AM.xls", firstrow(variables) replace
	    save 				"`counterfactual'_EXD3_`X'_AM.dta", replace
